...
frappe.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	// On item selection check is item has Alternative Item setup
	// This  indicator to allow to show or not a field in Setup Alt. Items
	// dialog (in BOM Line lvl)
	var d = locals[cdt][cdn];
...
cur_frm.select_bomline_alternate_items = function(opts) {
	...
	const d = new frappe.ui.Dialog({
		title: __("Select Alternate Items"),
		fields: [
			{
				fieldtype:'Link',
				...
				get_query: () => {
					var allready_selected = []
					for (var i = 0; i < cur_frm.alt_list_data.length; i++) {
						allready_selected.push(cur_frm.alt_list_data[i].alt_item)
					}
					return {
	...
	frappe.call({
		...
		callback:function(r){
			var bomlines_alt_items = r.message || [];
			cur_frm.alt_list_data = bomlines_alt_items
			d.fields_dict.alt_items.$wrapper.html(
				frappe.render_template(
				  'alternative_items_selector',
				  {'header_columns': headers, 'data': cur_frm.alt_list_data }
